...
import { signInGoogleAction, type AuthFormState } from "@/lib/actions/auth";

export function GoogleSignIn({ next }: { next: string }) {
  const [state, action, pending] = useActionState<AuthFormState, FormData>(signInGoogleAction, {});
  return <form action={action} className="googleSignIn">
    <input type="hidden" name="next" value={next} />
    <button type="submit" disabled={pending} className="googleButton">
      <svg width="20" height="20" viewBox="0 0 48 48" aria-hidden="true">
        <path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5Z"/>
        <path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.25 5.48-4.76 7.18l7.73 6c4.51-4.18 7.07-10.36 7.07-17.65Z"/>
      ...
      </svg>{pending ? "Connecting…" : "Continue with Google"}
    </button>
    {state.error && <p className="formError" role="alert">{state.error}</p>}
    <div className="authDivider">or use email</div>
  </form>;
}
