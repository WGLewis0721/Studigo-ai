...
import { ACTIVITIES, CHALLENGE_REQUESTS, LEARNING_ROUTES, nextChallenge, type ChallengeRequest, type LearningActivity, type LearningRoute } from '@/lib/learning';
import { loadConceptLearningState } from '@/lib/learning/persistence';

export const runtime = 'nodejs';

/** Read-only director boundary. Clients choose route/activity/stretch, never learner state. */
export async function GET(request: Request) {
  ...
  const challengeRequest = query.get('challengeRequest') ?? 'normal';
  ...
  if (!roomId || !topicId || !uuid.test(roomId) || !uuid.test(topicId)
    ...
    || !CHALLENGE_REQUESTS.includes(challengeRequest as ChallengeRequest)) {
    return Response.json({ error: 'Choose a valid room, concept, activity, route and challenge request.' }, { status: 400 });
  }
  ...
  try {
    ...
    const challenge = nextChallenge({ concept: { ...key, objective: topic.objective || topic.title },
      ...
      activity: activity as LearningActivity, route: route as LearningRoute,
      challengeRequest: challengeRequest as ChallengeRequest, now: new Date().toISOString() });
