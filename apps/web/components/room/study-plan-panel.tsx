...
export function StudyPlanPanel({roomId,days,testDate,onStart,onChanged,onSetDate}:{roomId:string;days:PlanDay[];testDate:string|null;onStart:(a:StudyAction)=>void;onChanged:()=>void;onSetDate:()=>void}) {
  ...
  return <section className="studyPlanPanel"><header className="studySectionHeading"><div><span className="tinyLabel">A LITTLE EACH DAY</span><h2>{testDate?`Your route to ${new Date(testDate).toLocaleDateString(undefined,{month:'short',day:'numeric'})}.`:'Make the next few days count.'}</h2><p>Recomputed from your current study scope, practice results and due cards. Skipped work returns to the plan; completed tasks stay recorded.</p></div><button className="buttonQuiet" onClick={onSetDate}>{testDate?'Change test date':'Set test date'} →</button></header>
