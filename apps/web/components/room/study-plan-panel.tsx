...
export function StudyPlanPanel({roomId,days,testDate,onStart,onChanged,onSetDate}:{roomId:string;days:PlanDay[];testDate:string|null;onStart:(a:StudyAction)=>void;onChanged:()=>void;onSetDate:()=>void}) {
  ...
  return <section className="studyPlanPanel"><header className="studySectionHeading"><div><span className="tinyLabel">A LITTLE EACH DAY</span><h2>{testDate?<><span className="planRouteLead">Your route to</span> <span className="planRouteDate">{new Date(testDate).toLocaleDateString(undefined,{month:'short',day:'numeric'})}.</span></>:'Make the next few days count.'}</h2><p>Recomputed from your current study scope, practice results and due cards. Skipped work returns to the plan; completed tasks stay recorded.</p></div><button className="buttonQuiet planDateButton" onClick={onSetDate} aria-label={testDate ? "Change test date" : "Set test date"}><span aria-hidden="true">◷</span><span>{testDate ? "Change date" : "Set date"}</span></button></header>
