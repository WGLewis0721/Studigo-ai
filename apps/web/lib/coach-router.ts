...
import type { ChallengeSpec, LearningRoute } from "@/lib/learning";
...
export async function* runCoachTurn(args: {
  ...
  const issueFor = async (topic: Topic): Promise<IssuedChallenge | undefined> => {
    if (!args.userId) return undefined;
    try {
      const spec = await deps.director.challengeFor({ supabase, userId: args.userId, roomId, topic, route });
      return { spec: spec as unknown as Record<string, unknown>, encounterId: randomUUID(), scaffoldUsed: spec.scaffoldLevel };
    } catch (error) {
  ...
  if (state.kind === "awaiting_control") {
    if (intent === "conversation_control") {
      ...
      // "Another one" asks the control plane again; the Coach never picks the next level.
      const issued = topic ? await issueFor(topic) : undefined;
      const log = yield* openCoachQuestion({ supabase, roomId, conversationId, topics, topic, deps, pedagogyDirectives, issued });
  ...
  // "Challenge me": a request for a fresh challenge, served by the control
  // plane. Whatever level it returns is the level asked; the Coach has no
  // difficulty ladder of its own and does not make questions harder by prompt.
  if (intent === "challenge") {
  ...
  if (state.kind === "awaiting_answer") {
    ...
    // reveal on the same encounter. The pending question is kept so the
    // learner can still attempt it, but no later answer can count as unaided.
    const reveal = async function* (resolvedIntent: string): AsyncGenerator<GroundedStreamEvent, CoachTurnLog> {
      ...
      await saveCoachState(supabase, conversationId, withSupport(state, SUPPORT_GIVEN.reveal));
    ...
    if (intent === "simplify") {
      ...
      const nextState: CoachState = withSupport({ ...state, question: simpler }, SUPPORT_GIVEN.simplify);
    ...
    if (intent === "example") {
      ...
      await saveCoachState(supabase, conversationId, withSupport(state, SUPPORT_GIVEN.example));
    ...
    if (effectiveIntent === "clarification") {
      ...
      // next turn is still routed as an answer to it rather than reset. The
      // answer to the learner's own question is assistance on this encounter.
      await saveCoachState(supabase, conversationId, withSupport(state, SUPPORT_GIVEN.clarification));
    ...
    if (outcome === "incorrect" || outcome === "irrelevant" || outcome === "help") {
      ...
      // correction) is recorded on the SAME encounter, from the resolved outcome.
      const support = feedbackSupport(outcome);
      if (support > 0 && state.issuedChallenge) {
        await saveCoachState(supabase, conversationId, withSupport(state, support));
      }
