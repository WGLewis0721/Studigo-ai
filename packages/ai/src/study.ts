...
export async function extractTopicMap(args: {
  ...
  const user = [
    ...
    asUntrustedMaterial(args.materialText.slice(0, 60_000))
  ].join("\n\n");
...
export async function gradeShortAnswer(args: {
  ...
  const user = [
    ...
    `Model answer: ${args.expectedAnswer}`,
...
export async function generateFlashcards(args: {
  ...
  const user = [
    ...
    args.topicTitle ? `Focus on the topic "${args.topicTitle}".` : "Cover the most testable ideas here.",
    `Numbered source excerpts:\n${asUntrustedMaterial(buildContextBlock(args.chunks))}`
  ].join("\n\n");
    ...
    input: [
      ...
      },
      {
        ...
        content: [
          ...
          args.objective ? `The learner must be able to: ${args.objective}` : "",
          `Numbered source excerpts:\n${asUntrustedMaterial(buildContextBlock(args.chunks))}`
        ]
          .filter(Boolean)
          .join("\n\n")
